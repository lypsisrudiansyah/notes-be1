...
clear
echo "Create and Manage Cloud Spanner Instances - GSP381"
...
echo "${BG_MAGENTA}${BOLD}Starting Execution -jaberw${RESET}"
...
touch jaberw
gsutil cp jaberw gs://$DEVSHELL_PROJECT_ID/tmp/jaberw
...
gcloud dataflow jobs run jaberw \
...
echo "${BG_RED}${BOLD}i think its success - ${RESET}"
