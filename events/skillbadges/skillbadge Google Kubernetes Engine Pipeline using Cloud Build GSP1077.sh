...
clear
echo "Google Kubernetes Engine Pipeline using Cloud Build  || GSP1077"
...
# Modern, high-vibrancy 256-color foreground palette
BLACK=`tput setaf 235`      # Sleek Charcoal / Jet Black (readable on dark terminals)
RED=`tput setaf 196`        # High-Voltage Crimson Red
GREEN=`tput setaf 48`       # Vibrant Spring / Mint Green
YELLOW=`tput setaf 220`     # Rich Golden Amber Yellow
BLUE=`tput setaf 39`        # Vivid Electric / Sky Blue
MAGENTA=`tput setaf 199`    # Cyberpunk Neon Violet / Pink
CYAN=`tput setaf 51`        # Neon Aqua / Ice Cyan
WHITE=`tput setaf 231`      # Pure Brilliant White

# Modern, rich 256-color background palette
BG_BLACK=`tput setab 234`   # Deep Matte Slate Black
BG_RED=`tput setab 124`     # Deep Velvet Wine Red
BG_GREEN=`tput setab 28`    # Lush Forest Green
BG_YELLOW=`tput setab 178`  # Warm Mustard Gold
BG_BLUE=`tput setab 25`     # Deep Royal Navy Blue
BG_MAGENTA=`tput setab 127` # Dark Plum / Purple
BG_CYAN=`tput setab 31`     # Deep Ocean Teal
BG_WHITE=`tput setab 255`   # Bright Canvas White
...
remove_files() {
    # 1. Enable nullglob so unmatched patterns expand to nothing instead of literal strings
    shopt -s nullglob

    # 2. Collect all matching regular files into an array
    local files=()
    ...
    # 3. If no matching files exist, exit cleanly
    if [[ ${#files[@]} -eq 0 ]]; then
        echo "No matching files found (gsp*, arc*, shell*)."
        return 0
    fi

    # 4. Display the files found for review
    echo "The following ${#files[@]} file(s) are targeted for deletion:"
    echo "--------------------------------------------------------"
    for f in "${files[@]}"; do
        echo "  - $f"
    done
    ...
    # 6. Proceed only on explicit 'y' or 'Y'
    if [[ "$confirm" =~ ^[Yy]$ ]]; then
        for f in "${files[@]}"; do
            rm -- "$f"
            echo "File removed: $f"
